##
##
## Expressed Transfac TFs
##
## After correcting the mappings (in badMaps.tsv )
##
annot.dir <- file.path(Sys.getenv("TFINF"),"data/expression")
...
gg <- read.table(paste(aux.dir,"factorsWithMatrices",sep="/"))
...
t1 <- read.table(paste(aux.dir,"MatrixToMouseGeneIDuniqueGeneID.tsv",sep="/"),as.is=TRUE);
allMats <- as.character(read.table(paste(aux.dir,"MouseMatrices",sep="/"),as.is=TRUE)$V1)
...
ofile <- paste(out.dir,"expressedFamilyMats.tsv",sep="/")
...
ofile <- paste(out.dir,"tteMaps.RData",sep="/")
