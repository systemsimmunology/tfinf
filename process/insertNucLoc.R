...
t1 <- read.table(paste(Sys.getenv("AUX_DIR"),"TFwesternScoredNuclear.tsv",sep="/"),as.is=TRUE,header=TRUE,row.names=1)
...
load(paste(Sys.getenv("EXP_DIR"),"all.mic.RData",sep="/"))
load(paste(Sys.getenv("EXP_DIR"),"scaled.mus.objects.RData",sep="/"))
load(paste(Sys.getenv("EXP_DIR"),"all.mus.objects.RData",sep="/"))
lps.mat.max1.exp <- lps.mat.max1 ## this one will retain expression vars (as opposed to nucloc variables)
...
ofile <- paste(Sys.getenv("PO_DIR"),"scaled.mus.objects.RData",sep="/")
