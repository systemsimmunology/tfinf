...
code.dir <- file.path(Sys.getenv("TFINF"),"R")

source(paste(code.dir,"selectionUtilities.R",sep="/"))
source(paste(code.dir,"utilitiesODEsolve.R",sep="/"))
source(paste(code.dir,"analyticODE.R",sep="/"))
...
load(paste(annot.dir,"expressed.scanned.ensembl.RData",sep="/")) ##targets used to be derived from pdnaModels. Now use all ese
...
save(collection.1reg,file="collection.1reg.RData")


#collection.1reg.sorted <- t(apply(collection.1reg,1,sort))
#targs.rands.1reg.0.05 <- collection.1reg.sorted[,5]
#targs.rands.1reg.0.10 <- collection.1reg.sorted[,10]
#targs.rands.1reg.0.20 <- collection.1reg.sorted[,20]
#targs.rands.1reg.0.50 <- collection.1reg.sorted[,50]
